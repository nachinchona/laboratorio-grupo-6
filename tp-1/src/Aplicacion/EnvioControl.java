...
public class EnvioControl extends Thread {
    ...
    private String[] mensajes = { "Hola", "Ciao", "Hello", "Bonjour", "Ni hao", "Annyeong", "Konnichiwa", "Hallo",
            "Zdravstvuyte" };
    ...
    public void run() {
        for (int i = 0; i < beeper.length; i++) {
            ScheduledFuture<?> beeperHandle = scheduler.scheduleWithFixedDelay(beeper[i], beeper[i].getDelay(),
                    beeper[i].getDelay(), TimeUnit.SECONDS);
